...
Pași de făcut la trecerea de la `protectie-electromagnetica.ro` (site vechi) la site-ul nou,
...
- [ ] Notează înregistrările actuale, la vechiul furnizor sau cu
      `dig MX protectie-electromagnetica.ro` / `dig TXT protectie-electromagnetica.ro`
...
$n = $db->query("SELECT COUNT(*) FROM blog_posts WHERE content LIKE \"%protectie-electromagnetica.ro%\"")->fetchColumn();
$m = $db->query("SELECT COUNT(*) FROM pages WHERE html_content LIKE \"%protectie-electromagnetica.ro%\"")->fetchColumn();
...
- [ ] `.env`: `APP_DEBUG=false`
- [ ] `.env`: `APP_URL=https://protectie-electromagnetica.ro`
...
- [ ] Așteptată propagarea (de la câteva minute la 24h): `dig protectie-electromagnetica.ro`
...
- [ ] `https://www.protectie-electromagnetica.ro` duce în același loc ca varianta fără `www`
...
for u in "/nume-articol-vechi/" "/category/nutritie/" "/tag/cancer/"; do
  echo "$(curl -s -o /dev/null -w '%{http_code} -> %{redirect_url}' "https://protectie-electromagnetica.ro$u")  $u"
done
```
...
```
*/5 * * * * wget -q -O /dev/null https://protectie-electromagnetica.ro/health
```
